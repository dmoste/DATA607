...
Finally, I created some plots using these new data frames to compare the airlines based on destination airports.
```{r}
...
### Conclusion
When I first compared the two airlines, AM West was shown to have a better on time rate than Alaska. However, when the data is broken down by destination airport, Alaska has a better on time rate for each individual destination. When you compare total flights to each destination, it becomes clear that the reason AM West has a better overall on time rate is due to the massive number of flights it sends to Phoenix and its high on time rate to that destination. In conclusion, while AM West has the higher overall on time rate, I would choose Alaska airlines since they still have a better on time rate at each airport (potentially unless I was headed to phoenix and I needed to have flexible times).